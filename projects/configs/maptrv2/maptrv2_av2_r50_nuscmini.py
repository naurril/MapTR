"""
MapTRv2-R50 Argoverse2 pretrained config for nuScenes-mini inference.

Uses the AV2-pretrained checkpoint (64.3 mAP on AV2 val).
AV2 was trained with 7 ring cameras; nuScenes-mini has 6 cameras.
num_cams=7 is kept to match checkpoint weights; use_cams_embeds=False
skips the 7-vs-6 embedding mismatch.

nuScenes images are 1600×900; scale=0.5 → 800×450.
LSSTransform uses explicit camera intrinsics so geometry is correct.
Note: AV2 pc_range is portrait-oriented vs nuScenes landscape;
results are geometrically valid but axes differ from nusc convention.
"""
_base_ = [
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            num_cams=7,            # must match checkpoint (AV2 has 7 ring cameras)
            use_cams_embeds=False, # nuScenes-mini has 6 cams; skip embedding to avoid 6 vs 7 mismatch
            z_cfg=z_cfg,
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_mini_map_infos_temporal_train.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_mini_map_infos_temporal_val.pkl',
