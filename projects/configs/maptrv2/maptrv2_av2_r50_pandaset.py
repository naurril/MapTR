"""
MapTRv2-R50 Argoverse2 pretrained config for PandaSet inference.

Uses the AV2-pretrained checkpoint (64.3 mAP on AV2 val).
AV2 was trained with 7 ring cameras; PandaSet has 6 cameras.
num_cams=7 is kept to match checkpoint weights — the extra cams_embed row
is never applied since LSSTransform does not use cams_embeds.

AV2 images were 2048×2048 at scale 0.3 → 614×614.
PandaSet images are 1920×1080; scale 0.3 → 576×324.
The LSSTransform uses explicit camera intrinsics so image size mismatch
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            use_cams_embeds=False, # PandaSet has 6 cams; skip embedding to avoid 6 vs 7 mismatch
            z_cfg=z_cfg,
...
# AV2 trained with scale=0.3 on 2048×2048 images → 614×614.
# PandaSet 1920×1080 × 0.3 → 576×324.
_img_scale_factor = 0.3
